...
{  
	...
	semaphores[index].val = value;

	for (int i=0; i<NLOCKS; i++) {
		initlock(&semaphores[index].lk, "temp");
	}
...
sys_sem_down(void)
{
	...
	spinlock_acquire(&semaphores[index].lk);

	while (semaphores[index].val <= 0) {
		cv_sleep((void *) (index+1), &semaphores[index].lk);
	}
	semaphores[index].val--;
	cprintf("Process: %d, sem[%d]: %d \n", myproc()->pid, index, semaphores[index].val);

	spinlock_release(&semaphores[index].lk);
