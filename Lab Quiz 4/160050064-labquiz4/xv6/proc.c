...
struct taskmaster_t
{
	int cv1;				// for master process by child to know work completed.
	int cv2;				// for child process by master to know new work created.
	//	.......
...
int help_wait_for_task(void) {
	...
	while (1) {
		...
		if (pending_task_index == -1) cond_wait(&taskmaster.cv2, &taskmaster.lock, -1);
		else break;
	}
